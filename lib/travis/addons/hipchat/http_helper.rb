  ...
  module Addons
    module Hipchat
      class HttpHelper
        ...
        DEFAULT_SERVER = 'api.hipchat.com'

        attr_reader :api_version, :headers, :url, :token, :room_id

        def initialize(specification, server)
          ...
          when API_V1_TOKEN_LENGTH
            @api_version = 'v1'
            @url = 'https://%s/v1/rooms/message?format=json&auth_token=%s' % [server, token]
            @headers = {}
          when API_V2_TOKEN_LENGTH
            @api_version = 'v2'
            @url = 'https://%s/v2/room/%s/notification?auth_token=%s' % [server, encode(room_id), token]
